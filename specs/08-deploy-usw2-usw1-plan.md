# Spec 08 — Deploy Regions us-west-2 and us-east-1

**Status**: Complete

## Overview

Deploy a full us-west-2 (Oregon) and us-east-1 (N. Virginia) regional stack to mirror what is currently running in eu-west-2 and eu-west-1. This includes:
...
| vpc-use1-prod-cell6000 | prod | 10.48.0.0/20 | us-east-1 |
| vpc-use1-prod-cell6001 | prod | 10.48.16.0/20 | us-east-1 |
| vpc-use1-dev-cell7000 | dev | 10.49.0.0/20 | us-east-1 |
| vpc-use1-dev-cell7001 | dev | 10.49.16.0/20 | us-east-1 |
...
| use1 | prod | 10.48.0.0/16 |
| use1 | dev | 10.49.0.0/16 |
...
| `envs/dev/use1/keypair/` | `env-dev/use1/keypair/terraform.tfstate` |
| `envs/prod/use1/keypair/` | `env-prod/use1/keypair/terraform.tfstate` |
| `envs/networking/usw2/tgw/` | `env-networking/usw2-tgw/terraform.tfstate` |
| `envs/networking/use1/tgw/` | `env-networking/use1-tgw/terraform.tfstate` |
...
| `envs/prod/use1/cell6000/` | `env-prod/use1/cell6000/terraform.tfstate` |
| `envs/prod/use1/cell6001/` | `env-prod/use1/cell6001/terraform.tfstate` |
| `envs/dev/use1/cell7000/` | `env-dev/use1/cell7000/terraform.tfstate` |
| `envs/dev/use1/cell7001/` | `env-dev/use1/cell7001/terraform.tfstate` |
| `envs/networking/usw2/tgw-vpc-atts/` | `env-networking/usw2-tgw-vpc-atts/terraform.tfstate` |
| `envs/networking/use1/tgw-vpc-atts/` | `env-networking/use1-tgw-vpc-atts/terraform.tfstate` |
...
| `tgw-att-euw2-use1-pcx` | euw2 | use1 | Yes |
| `tgw-att-euw1-usw2-pcx` | euw1 | usw2 | Yes |
| `tgw-att-euw1-use1-pcx` | euw1 | use1 | Yes |
| `tgw-att-usw2-use1-pcx` | usw2 | use1 | Yes |
...
The existing `modules/security` module accepts a single `cross_region_supernet_cidr` string. usw2 and use1 cells peer with three other regions each, so the module must be extended to accept a list.
...
- `envs/dev/use1/keypair/`
- `envs/prod/use1/keypair/`
...
- `envs/prod/use1/cell6000/`, `cell6001/`
- `envs/dev/use1/cell7000/`, `cell7001/`
...
Variants: `prod/usw2` → `environment = "prod"`, `dev/use1` → `region = "us-east-1"`, `region_short = "use1"`, `prod/use1` → both.
...
**`envs/networking/use1/tgw/locals.tf`**: `region = "us-east-1"`, `region_short = "use1"`, `amazon_side_asn = 64517`.
...
**us-east-1 AZ constraint**: Only two AZs exist — `us-east-1a` and `us-east-1b`. use1 cells have only `priv-0` and `priv-1` (no `priv-2`/`pub-2`). The `create-vpc` module uses `for_each` on the subnets map, so variable counts are supported without code changes.

**`security.tf` for use1-prod cells**:
...
**`security.tf` for use1-dev cells**:
...
**`envs/networking/use1/tgw-vpc-atts/locals.tf`**: same pattern with cell6000/6001 (prod) and cell7000/7001 (dev).
...
**`tgw-peering.tf`** — extend locals and add 5 new peering blocks (euw2↔usw2, euw2↔use1, euw1↔usw2, euw1↔use1, usw2↔use1) each following the existing euw2↔euw1 pattern:
...
| euw2 | prod | 10.48.0.0/16 | euw2↔use1 (new) |
| euw2 | dev | 10.17.0.0/16 | euw2↔euw1 (existing) |
| euw2 | dev | 10.33.0.0/16 | euw2↔usw2 (new) |
| euw2 | dev | 10.49.0.0/16 | euw2↔use1 (new) |
| euw1 | prod | 10.0.0.0/16 | euw2↔euw1 (existing, accepter) |
| euw1 | prod | 10.32.0.0/16 | euw1↔usw2 (new) |
| euw1 | prod | 10.48.0.0/16 | euw1↔use1 (new) |
| euw1 | dev | 10.1.0.0/16 | euw2↔euw1 (existing, accepter) |
| euw1 | dev | 10.33.0.0/16 | euw1↔usw2 (new) |
| euw1 | dev | 10.49.0.0/16 | euw1↔use1 (new) |
| usw2 | prod | 10.0.0.0/16 | euw2↔usw2 (accepter) |
| usw2 | prod | 10.16.0.0/16 | euw1↔usw2 (accepter) |
| usw2 | prod | 10.48.0.0/16 | usw2↔use1 (new) |
| usw2 | dev | 10.1.0.0/16 | euw2↔usw2 (accepter) |
| usw2 | dev | 10.17.0.0/16 | euw1↔usw2 (accepter) |
| usw2 | dev | 10.49.0.0/16 | usw2↔use1 (new) |
| use1 | prod | 10.0.0.0/16 | euw2↔use1 (accepter) |
| use1 | prod | 10.16.0.0/16 | euw1↔use1 (accepter) |
| use1 | prod | 10.32.0.0/16 | usw2↔use1 (accepter) |
| use1 | dev | 10.1.0.0/16 | euw2↔use1 (accepter) |
| use1 | dev | 10.17.0.0/16 | euw1↔use1 (accepter) |
| use1 | dev | 10.33.0.0/16 | usw2↔use1 (accepter) |
...
|---|---|
| `discover_keypairs()` | `envs/dev/usw2/keypair/`, `envs/prod/usw2/keypair/`, `envs/dev/use1/keypair/`, `envs/prod/use1/keypair/` |
| `discover_tgws()` | `envs/networking/usw2/tgw/`, `envs/networking/use1/tgw/` |
| `discover_vpc_cells()` | All eight new cell directories |
| `discover_tgw_vpc_atts()` | `envs/networking/usw2/tgw-vpc-atts/`, `envs/networking/use1/tgw-vpc-atts/` |
| `discover_tgw_peering()` | Already returns `envs/networking/global/tgw-peering/` |

The peering gate (`peering_gate.py`) uses the regex `r'key\s*=\s*"([^"]*-tgw[^"]*)"'` which already matches `usw2-tgw` and `use1-tgw` keys. Once the two new `terraform_remote_state` blocks are added to `data.tf`, the gate will automatically include all four regions.
...
- `discover_vpc_cells` picks up usw2 and use1
- `discover_tgws` picks up usw2 and use1
- `discover_keypairs` picks up usw2 and use1
- Region filter `--regions usw2` excludes use1, euw2, euw1
...
python scripts/deploy.py --regions usw2,use1 --dry-run

# Incremental: deploy TGWs+VPCs first, skip peering
python scripts/deploy.py --regions usw2,use1 --skip-peering
...
python scripts/destroy.py --regions usw2,use1 --dry-run
```
...
#   Phase 3: peering gate checks euw2, euw1, usw2, use1
```
...
for dir in \
  ...
  envs/prod/use1/cell6000 envs/prod/use1/cell6001 \
  envs/dev/use1/cell7000  envs/dev/use1/cell7001 \
  envs/networking/usw2/tgw-vpc-atts \
  envs/networking/use1/tgw-vpc-atts \
...
2. SSH to bastion in use1-dev (cell7000) → ping usw2-dev (`10.33.0.x`) — direct use1↔usw2 path
3. SSH to bastion in use1-dev (cell7000) → ping euw1-dev (`10.17.0.x`) — direct use1↔euw1 path
...
**us-east-1 AZ count.** us-east-1 has only two AZs (`us-east-1a`, `us-east-1b`). use1 cells must define only `priv-0` and `priv-1` (no `priv-2`/`pub-2`). The `create-vpc` module uses `for_each`, so this works without module changes.
...
**120-second `time_sleep` per new peering attachment.** Five new `time_sleep` resources will be created. Terraform may parallelise independent waits (e.g. euw1↔usw2 and usw2↔use1 have no inter-dependency), reducing total apply time.

**State key convention for peering gate.** The gate regex `r"env-networking/([^/]+)-tgw/"` extracts `usw2` from `env-networking/usw2-tgw/terraform.tfstate` and constructs path `envs/networking/usw2/tgw`. Directory names must use `usw2`/`use1` short-names consistently.
...
| `envs/dev/use1/keypair/` | same |
| `envs/prod/use1/keypair/` | same |
| `envs/networking/usw2/tgw/` | `backend.tf`, `locals.tf`, `providers.tf`, `tgw.tf`, `outputs.tf` |
| `envs/networking/use1/tgw/` | same |
...
| `envs/prod/use1/cell6000/` | same |
| `envs/prod/use1/cell6001/` | same |
| `envs/dev/use1/cell7000/` | same |
| `envs/dev/use1/cell7001/` | same |
| `envs/networking/usw2/tgw-vpc-atts/` | `backend.tf`, `locals.tf`, `providers.tf`, `variables.tf`, `data.tf`, `vpc-attachments.tf`, `outputs.tf` |
| `envs/networking/use1/tgw-vpc-atts/` | same |
...
| `envs/networking/global/tgw-peering/providers.tf` | Add `aws.usw2` and `aws.use1` aliases |
| `envs/networking/global/tgw-peering/data.tf` | Add `usw2_tgw` and `use1_tgw` remote state blocks |
...
- [x] **Remove `moved` blocks** from `envs/networking/global/tgw-peering/tgw-peering.tf` — removed after successful apply
- [x] **JSON instance inventory export** — `deploy.py` writes `instances.json` to repo root after deploy; added to `.gitignore`
